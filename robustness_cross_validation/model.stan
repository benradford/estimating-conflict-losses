...
data {
  ...
  int<lower=1> cum_num_obs_oos;       // cumulative number of observations
  int<lower=1> daily_num_obs_oos;     // daily number of obs
  
  // Cumulative covariates and outcomes
  int<lower=1> cum_day_oos[cum_num_obs_oos];              // day index
  int<lower=0> cum_count_oos[cum_num_obs_oos];            // cumulative losses
  int<lower=1> cum_loss_type_oos[cum_num_obs_oos];        // loss type
  int<lower=1> cum_claim_source_oos[cum_num_obs_oos];     // claim source
  int<lower=1> cum_loss_country_oos[cum_num_obs_oos];
  int<lower=1> cum_category_oos[cum_num_obs_oos];
  int<lower=0,upper=1> cum_min_oos[cum_num_obs_oos];              // minimum estimate
  int<lower=0,upper=1> cum_max_oos[cum_num_obs_oos];              // maximum estimate

  // Daily covariates and oucomes
  int<lower=0> daily_count_oos[daily_num_obs_oos];        // daily losses
  int<lower=1> daily_day_oos[daily_num_obs_oos];          // daily day index
  int<lower=1> daily_loss_type_oos[daily_num_obs_oos];    // daily loss type
  int<lower=1> daily_claim_source_oos[daily_num_obs_oos]; // daily claim source
  int<lower=1> daily_loss_country_oos[daily_num_obs_oos];
  int<lower=1> daily_category_oos[daily_num_obs_oos];
  int<lower=0,upper=1> daily_min_oos[daily_num_obs_oos];  // minimum estimate
  int<lower=0,upper=1> daily_max_oos[daily_num_obs_oos];  // maximum estimate
}
...
generated quantities {
  ...
  real obs_cum_pos_pred_oos[cum_num_obs_oos];
  real obs_daily_pos_pred_oos[daily_num_obs_oos];
  ...
  for(ii in 1:daily_num_obs_oos)
  {
    obs_daily_pos_pred_oos[ii] = poisson_log_rng(latent_losses[daily_loss_type_oos[ii],
                                     daily_day_oos[ii]] +
                       bias_source_type_mu[daily_claim_source_oos[ii], daily_category_oos[ii] + (daily_loss_country_oos[ii]-1)*num_category] +
                       beta_min[daily_claim_source_oos[ii]]*daily_min_oos[ii] +
                       beta_max[daily_claim_source_oos[ii]]*daily_max_oos[ii]);

  }
  // Cumulative observed losses expected values
  for(ii in 1:cum_num_obs_oos)
  {
    obs_cum_pos_pred_oos[ii] =  neg_binomial_2_log_rng(cum_losses[cum_loss_type_oos[ii],
                                 cum_day_oos[ii]] + 
                      bias_source_type_mu[cum_claim_source_oos[ii], cum_category_oos[ii] + (cum_loss_country_oos[ii]-1)*num_category] +
                      beta_min[cum_claim_source_oos[ii]]*cum_min_oos[ii] +
                      beta_max[cum_claim_source_oos[ii]]*cum_max_oos[ii],
                      inv_cum_phi[cum_loss_type_oos[ii]]);
  }
}
