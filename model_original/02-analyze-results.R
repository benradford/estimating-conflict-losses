...
############################################
## BAYES PLOT POSTERIOR PREDICTIVE CHECKS ##
############################################
# png("post_pred_cum.png", width=900, height=450)
ppc_dens_overlay(asinh(data$loss_count_min),
                 asinh(obs_cum_post_pred[sample(1:dim(obs_cum_post_pred)[1], 2000),]))
# dev.off()
# png("post_pred_daily.png", width=900, height=450)
ppc_dens_overlay(asinh(data_daily$loss_count_min),
                 asinh(obs_daily_post_pred[sample(1:dim(obs_daily_post_pred)[1], 2000),]))
# dev.off()
...
##############################################
print("NumObs")
print(standata[["daily_num_obs"]] + standata[["cum_num_obs"]])
print("PercentCum")
print( standata[["cum_num_obs"]] / (standata[["daily_num_obs"]] + standata[["cum_num_obs"]]))
print("NumTypes")
print(length(levels(data$category)))

print("RussianLosses")
print(mean(exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Military Deaths-Russia"),365])))
print("RussianLossesCI")
print(quantile(exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Military Deaths-Russia"),365]), probs=c(0.025,0.975)))
print("UkrainianLosses")
print(mean(exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Military Deaths-Ukraine"),365])))
print("UkrainianLossesCI")
print(quantile(exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Military Deaths-Ukraine"),365]), probs=c(0.025,0.975)))

#############################################
## LOSS RATIOS                             ##
#############################################
troop_loss_ratio_ua <- exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Military Casualties-Ukraine"),365]) / exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Military Deaths-Ukraine"),365])
troop_loss_ratio_ru <- exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Military Casualties-Russia"),365]) / exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Military Deaths-Russia"),365])
print("RussianCasualtiesToLosses")
mean(troop_loss_ratio_ru)
quantile(troop_loss_ratio_ru, probs=c(0.025,0.975))
print("UkrainianCasualtiesToLosses")
mean(troop_loss_ratio_ua)
quantile(troop_loss_ratio_ua, probs=c(0.025,0.975))

troop_loss_ratio_ua <- (exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Military Injuries-Ukraine"),365]) + exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Military Deaths-Ukraine"),365])) / exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Military Deaths-Ukraine"),365])
troop_loss_ratio_ru <- (exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Military Injuries-Russia"),365]) + exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Military Deaths-Russia"),365])) / exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Military Deaths-Russia"),365])
print("RussianInjuriesToLosses")
mean(troop_loss_ratio_ru)
quantile(troop_loss_ratio_ru, probs=c(0.025,0.975))
print("UkrainianInjuriesToLosses")
mean(troop_loss_ratio_ua)
quantile(troop_loss_ratio_ua, probs=c(0.025,0.975))

troop_loss_ratio_ru_ua <- exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Military Deaths-Russia"),365]) / exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Military Deaths-Ukraine"),365])
print("RussianUkrainianRatio")
mean(troop_loss_ratio_ru_ua)
print("RussianUkrainianRatioCI")
quantile(troop_loss_ratio_ru_ua, probs=c(0.025,0.975))

#############################################
## TANK LOSSES                             ##
#############################################
tanks_ru <- exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Tanks-Russia"),365])
print("RussianTankLosses")
mean(tanks_ru)
quantile(tanks_ru, probs=c(0.025,0.975))

tanks_ua <- exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Tanks-Ukraine"),365])
print("UkrainTankLosses")
mean(tanks_ua)
print("UkrainTankLossesCI")
quantile(tanks_ua, probs=c(0.025,0.975))

##############################################
## SEPTEMBER 21, 2022                       ##
##############################################
sept21 <- exp(unbiased_cum_mean[,which.max(levels(data$loss_type)=="Military Deaths-Russia"),209]) 
mean(sept21)
quantile(sept21, probs=c(0.025,0.975))

##############################################
## BIASES                                   ##
##############################################
print("Russian Bias of Russian Deaths")
print(bias_mean["RU Source","Military Deaths-Russia"])
print(bias_lc["RU Source","Military Deaths-Russia"])
print(bias_uc["RU Source","Military Deaths-Russia"])

print("Russian Bias of Ukrainian Deaths")
print(bias_mean["RU Source","Military Deaths-Ukraine"])
print(bias_lc["RU Source","Military Deaths-Ukraine"])
print(bias_uc["RU Source","Military Deaths-Ukraine"])

print("Ukrainian Bias of Russian Deaths")
print(bias_mean["UA Source","Military Deaths-Russia"])
print(bias_lc["UA Source","Military Deaths-Russia"])
print(bias_uc["UA Source","Military Deaths-Russia"])

print("Ukrainian Bias of Ukrainian Deaths")
print(bias_mean["UA Source","Military Deaths-Ukraine"])
print(bias_lc["UA Source","Military Deaths-Ukraine"])
print(bias_uc["UA Source","Military Deaths-Ukraine"])

##############################################
## CORRELATIONS                             ##
##############################################
rownames(Omega_mean) <- colnames(Omega_mean) <- levels(data$loss_type)
rownames(Omega_lc) <- colnames(Omega_lc) <- levels(data$loss_type)
rownames(Omega_uc) <- colnames(Omega_uc) <- levels(data$loss_type)
diag(Omega_mean) <- diag(Omega_lc) <- diag(Omega_uc) <- 0

max(Omega_mean)
Omega_mean == max(Omega_mean)
Omega_lc["Tanks-Russia","Armored Vehicles-Russia"]
min(Omega_mean)
Omega_mean == min(Omega_mean)

Omega_uc["Armored Vehicles-Russia","Military Injuries-Russia"]
Omega_lc["Armored Vehicles-Russia","Military Injuries-Russia"]
...
table(data$loss_country) + table(data_daily$loss_country)
xtable(table(data$loss_country,
               data$claim_source) 
         + table(data_daily$loss_country, 
                 data_daily$claim_source))

##############################################
## RUSSIAN AND UKRAINIAN OBSERVATIONS       ##
##############################################
table(data$claim_source) + table(data_daily$claim_source)
