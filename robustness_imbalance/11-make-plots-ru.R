...
bias <- read.csv("biases_ru.csv",header=F,stringsAsFactors=F)
cum_losses <- read.csv("cum_losses_ru.csv",header=F,stringsAsFactors=F)
...
## BIAS BOXPLOTS                            ##
##############################################
pdf("ru_rep_bias.pdf",width=9,height=3)
...
## CUM LOSSES BOXPLOTS                      ##
##############################################
pdf("ru_rep_loss.pdf",width=9,height=3)
par(mfrow=c(1,1), mar=c(2.1,4.1,0.1,0.1))
plot(1:20,1:20,
     xlim=c(1,20),
     ylim=c(4,13),
