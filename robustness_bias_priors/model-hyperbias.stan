  ...
  for(cs in 1:num_claim_source){
    for(tt in 1:num_category){
      for(lc in 1:num_loss_country){
        bias_source_type_mu[cs,tt+(lc-1)*num_category] = (bias_mu+bias_source_offset[cs,lc]*bias_sigma) + (bias_source_type_offset[cs,tt+(lc-1)*num_category] * bias_source_sigma[cs,lc]);
      }
    }
  }
