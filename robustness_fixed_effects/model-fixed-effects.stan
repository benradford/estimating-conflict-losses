...
transformed parameters{
  ...
  // BIASE FE
  // matrix[num_claim_source, num_loss_country]             bias_fe;
  // bias_fe[1,1] = 0;
  // bias_fe[2:num_claim_source,1] = to_vector(bias_source_offset[1:(num_claim_source-1)]);
  // bias_fe[1:num_claim_source,2] = to_vector(bias_source_offset[(num_claim_source):(num_claim_source * num_loss_country - 1)]);
  
  matrix[num_claim_source, num_loss_country]             bias_fe;
  bias_fe = bias_source_offset;
  // bias_fe[5,2] = 0; ## UA ESTIMATES OF UA MILITARY DEATHS IS THE OMITTED CATEGORY
  bias_fe[7,1] = 0;
  bias_fe[7,2] = 0;
  
  // for(ii in 1:num_claim_source){
  //   for(jj in 1:num_loss_country){
  //     if(ii == 1 && jj == 1){
  //       
  //     } else{
  //       bias_fe[ii,jj] = bias_source_offset[(ii-1)+((jj-1)+(num_claim_source))];
  //     }
  //   }
  // }
  ...
  for(lc in 1:num_loss_country){
    for(cs in 1:num_claim_source){
      for(tt in 1:num_category){
        bias_source_type_mu[cs,tt+(lc-1)*num_category] = (bias_fe[cs,lc]) + (bias_source_type_offset[cs,tt+(lc-1)*num_category] * bias_source_sigma[cs,lc]);
      }
    }
  }
